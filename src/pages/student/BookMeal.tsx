  ...
  const handleBookMeal = async () => {
    if (!user) {
      toast({
        variant: 'destructive',
        title: 'Error',
        description: 'You must be logged in to book a meal',
      });
      return;
    }
    ...
    if (isBefore(startOfDay(selectedDate), startOfDay(new Date()))) {
      toast({
        variant: 'destructive',
        title: 'Invalid Date',
        description: 'Cannot book meals for past dates',
      });
      return;
    }
    ...
    if (existingBooking) {
      toast({
        ...
        description: 'You already have a booking for this meal type on this date',
      });
      return;
    }
    ...
    try {
      ...
      toast({
        title: 'Success!',
        description: `Your ${selectedMealType} has been booked for ${format(selectedDate, 'MMMM d, yyyy')}`,
      });
      ...
      toast({
        ...
        description: error.message || 'Failed to book meal',
      });
    } finally {
      setBooking(false);
    }
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <Card className="card-shadow">
              <CardHeader>
                ...
                <CardDescription>
                  Available {selectedMealType} items for {format(selectedDate, 'MMM d')}
                </CardDescription>
              </CardHeader>
              <CardContent>
                {loading ? (
                  ...
                  <div className="text-center py-8">
                    ...
                    <p className="text-muted-foreground">
                      No menu items available for {selectedMealType}
                    </p>
                  </div>
                ) : (
                  <div className="grid gap-3 sm:grid-cols-2">
                    {menuItems.map((item) => (
                      <button
                        ...
                        onClick={() =>
                          setSelectedMenuItem(
                            selectedMenuItem === item.id ? null : item.id
                          )
                        }
                        className={cn(
            ...
            <Button
              ...
              disabled={booking || existingBooking || isPastDate}
